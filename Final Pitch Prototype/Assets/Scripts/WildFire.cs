...
public class WildFire : MonoBehaviour {
	...
	{
		if(Input.GetButton("Fire1")) //GetButton has to be used as it's the only one that doesn't have its flag reset each frame
		{
			creatingFire = true;
			for(int radius = 0; radius < 10; radius++)
			{
				for (int i = 0; i < numberOfObjects; i++)
				{
					...
					Vector3 pos = (new Vector3(Mathf.Cos(angle), 0, Mathf.Sin(angle)) * radius) + new Vector3(25.0f, 0, 20.0f); //Put in centre of arena
