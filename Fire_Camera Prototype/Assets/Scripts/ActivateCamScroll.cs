...
public class ActivateCamScroll : MonoBehaviour {
	...
	void Update () 
	{
		if(moving == true)
		{
			if(!adventure)
			{
				activateCameraScrolling.MoveCamera(newCameraPosition); //Using the CamScroll script, move the camera to its new position

				//If the camera has reached its destination then stop moving the camera
				if(camera.transform.position == GameObject.Find(newCameraPosition).transform.position)
				{
					moving = false;
				}
			}
		...
		cameraLookAt = lookAtMe.position + (gameObject.transform.position - lookAtMe.position)/2;
