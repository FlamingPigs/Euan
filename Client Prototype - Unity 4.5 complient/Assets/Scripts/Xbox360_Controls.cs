...
public class Xbox360_Controls : MonoBehaviour 
{
	...
	//my additions
	//EDIT
	public Interact_Object_Controller IOC;
	private float start_hold;
	void Start()
	{
	...
	void UserInputs()
	{
		if(playerAnimator)
		{
			...
			//EDIT
			//more additions
			// these two functions SHOULD prevent the layer picking up an object
			//when not holding Y
			//by making the value of time held reset apon letting go
			/*if (Input.getButtonDown(gamepad[playerController, 3]))
			{
				start_hold=Time.time ;
				IOC.Interact(start_hold);
			}
			if(Input.GetButtonUp(gamepad[playerController, 3]))
			   {
				start_hold = 0;
				IOC.Interact(start_hold);
				}
